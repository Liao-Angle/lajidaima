...
Set objLogFile = objFileSystem.OpenTextFile("d:\ECP\WebFormPT\Batch\log\SmpPrePareDocAlert1.Log", 8, True) '8 is for appending
...
strSql = strSql & " , ya.SMWYAAA012+'-'+ ya.SMWYAAA013 relationShipMember , ur.mailAddress relaEmail ,ya.SMWYAAA017 "
strSql = strSql & " , wi.workItemName,u.userName,wi.createdTime, ya.SMWYAAA005 "
strSql = strSql & " from  [NaNa].dbo.WorkItem wi, [NaNa].dbo.ProcessInstance pi, [WebFormPT].[dbo].SMWYAAA ya , [NaNa].dbo.WorkAssignment wa , [NaNa].dbo.Users u, [NaNa].dbo.Users uw, [NaNa].dbo.Users ur, [WebFormPT].dbo.EmployeeInfo ei "
strSql = strSql & " where wi.contextOID=pi.contextOID and  pi.serialNumber=ya.SMWYAAA005  and wa.workItemOID=wi.OID  and u.id='1356' and wi.completedTime is null and ya.SMWYAAA020='I' "
strSql = strSql & " and u.OID=wa.assigneeOID and uw.id= ya.SMWYAAA008 and ur.id=ya.SMWYAAA012 and ya.SMWYAAA004 <>'請假單流程' and ya.SMWYAAA004 <>'出勤修正單流程' and  ya.SMWYAAA004 <>'費用請款單流程' and ei.empGUID = ur.OID  "
strSql = strSql & " union all "
strSql = strSql & " select  ei.orgId, ya.SMWYAAA002 , ya.SMWYAAA004 , ya.SMWYAAA006  "
strSql = strSql & " , ya.SMWYAAA008+'-'+ ya.SMWYAAA009 writeMember, uw.mailAddress writeEmail  "
strSql = strSql & " , ur.id+'-'+ ur.userName relationShipMember , ur.mailAddress relaEmail ,ya.SMWYAAA017  "
strSql = strSql & " , wi.workItemName,u.userName,wi.createdTime, ya.SMWYAAA005 "
strSql = strSql & " from  [NaNa].dbo.WorkItem wi, [NaNa].dbo.ProcessInstance pi, [WebFormPT].[dbo].SMWYAAA ya , [NaNa].dbo.WorkAssignment wa , [NaNa].dbo.Users u, [NaNa].dbo.Users uw, [NaNa].dbo.Users ur, [WebFormPT].dbo.SmpExpenseBill eb, [WebFormPT].dbo.EmployeeInfo ei "
strSql = strSql & " where wi.contextOID=pi.contextOID and  pi.serialNumber=ya.SMWYAAA005  and wa.workItemOID=wi.OID  and u.id='1356' and wi.completedTime is null and ya.SMWYAAA020='I' "
strSql = strSql & "   and u.OID=wa.assigneeOID and uw.id= ya.SMWYAAA008 and ur.OID=eb.PurMemberGUID and  ya.SMWYAAA004 ='費用請款單流程' and eb.SheetNo = ya.SMWYAAA002 and ei.empGUID = ur.OID  "
...
If not sqlEC.eof Then
	While not sqlEC.eof
		'intCnt = intCnt + 1
		strText = "<font lang=""EN-US"" style=""font-size: 9pt; font-family: Tahoma"">[至今日 <"&str_today&"> 請列印後送至董事長室單據]"
		...
		strText = strText & "<tr style=""font-size: 9pt; font-family: Tahoma""><td>公司別</td><td>單別</td><td>單號</td><td>填表人</td><td>關係人</td><td>填表日期</td><td>董事長收件日</td><td>主旨</td></tr>"		
		strText = strText & "<tr style=""font-size: 9pt; font-family: Tahoma""><td bgcolor=""#FFFFFF"" width=30>" & sqlEC("orgId") & "</td>"
		strText = strText & "<td bgcolor=""#FFFFFF"" width=80>" & sqlEC("SMWYAAA004") & "</td>"
		...
		With cdoMessage
			...
			'.Cc = "eva_fan@simplo.com.tw;Grace_Fan@simplo.com.tw"
			.Subject = "[New EasyFlow通知] <"&str_today&">需列印後送至董事長室單據"
			.htmlbody = strText
			.Send
		End With 
