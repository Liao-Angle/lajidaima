...
public partial class SmpProgram_Maintain_OracleAttachment_Download : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            //string flag = chkProxy(engineOra); //判斷代理人權限
            //int auth = 0;
            //if ((auth == 0) && (flag == "N"))
            if (auth == 0)
                Response.Redirect("~/NoAuth.aspx");
    ...
    //protected string chkProxy(AbstractEngine engine)
    //{
    //    string result = "N";
    //    string userID = (string)Session["UserID"];
    //    string sql = "select id, 'all' reportName from SMVKAAA, SMVKAAB, Users " +
    //                 " where SMVKAAA002=(select OID from Users where id='" + userID + "') and SMVKAAB002=SMVKAAA001 and OID=SMVKAAB005 " +
    //                 "   and ( convert(varchar, getdate(), 111) + ' ' + convert(varchar, getdate(), 108) between SMVKAAA003 and SMVKAAA004) "+
    //                 " union  "+
    //                 "select distinct id, SMVKAAC007 as reportName from SMVKAAA, SMVKAAC, Users "+
    //                 " where SMVKAAA002=(select OID from Users where id='" + userID + "') and SMVKAAC002=SMVKAAA001 and OID=SMVKAAC004 " +
    //                 "   and ( convert(varchar, getdate(), 111) + ' ' + convert(varchar, getdate(), 108) between SMVKAAA003 and SMVKAAA004)";
    //    DataSet ds = engine.getDataSet(sql, "TEMP");
    //    int rows = ds.Tables[0].Rows.Count;
    //    if (rows > 0)
