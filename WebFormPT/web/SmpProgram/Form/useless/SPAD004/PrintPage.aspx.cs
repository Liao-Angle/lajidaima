...
public partial class SmpProgram_Form_SPAD004_PrintPage : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
			...
			sql = sql + ",case u.orgId when 'SMP' then '新普科技'  when 'TP' then '中普科技' else '嘉普科技' end as OrgName ";
			...
			try
			{
				...
				sql = sql + "and wi.workItemName in ('ChargeSTCS','ChargeSCQ','審核人','直屬主管','審核人','董事長','處級','處級主管','部級','特別助理','特助','總經理室','總經理') ";
				sql = sql + "and substring(SMWYAAA002,1,7) in ('SPAD004') ";
				sql = sql + "and ya.SMWYAAA002='" + Utility.filter(strSheetNo) + "'  ";
				*/
				sql = "select  ROW_NUMBER() OVER(ORDER BY  McompletedTime) AS ROWID, userId, signUserName, workItemName, McompletedTime as completedTime, imgName, comment, reassign  from ( ";
				sql = sql + "select userId, signUserName, workItemName, max(completedTime) McompletedTime, imgName, comment, reassign from ( ";
				sql = sql + "select ROW_NUMBER() OVER(ORDER BY  wi.completedTime) AS ROWID, ur.id userId, ur.userName as signUserName, wi.workItemName ,CONVERT(VARCHAR, wi.completedTime, 120 ) completedTime , fi.FILENAME+'.'+fi.FILEEXT imgName ";
				sql = sql + " , replace(CAST(wi.executiveComment AS nvarchar(255)),'同意##','') as comment  ";
				sql = sql + " , dbo.SmpGetReassignUser(wi.OID ,pi.OID) as reassign ";
				sql = sql + "from NaNa.dbo.WorkItem wi, NaNa.dbo.ProcessInstance pi, WebFormPT.dbo.SMWYAAA ya, NaNa.dbo.Users ur , WebFormPT.dbo.FILEITEM fi  ";
				sql = sql + "where wi.contextOID=pi.contextOID ";
				...
				sql = sql + "and substring(SMWYAAA002,1,7) in ('SPAD004') ";
				sql = sql + "and ya.SMWYAAA002='" + Utility.filter(strSheetNo) + "'  ";
				sql = sql + " and wi.currentState='3' ) aa group by  userId, signUserName, workItemName, imgName, comment, reassign ";
				sql = sql + " ) abcd ";
				ds = engine.getDataSet(sql, "TEMP");
