    ...
    public string createSupplierEvaluateFlow(string docId, string userId, string subject, string htmlContent, string isNewContract, string password)
    {
        ...
        if (password.Equals(servicePwd))
        {
            ...
            try
            {
                ...
                //取得主管
                sql = "select u.id from Functions f, Users u where f.occupantOID = '" + Utility.filter(originatorGUID) + "' and f.specifiedManagerOID = u.OID";
                managerId = (string)engine.executeScalar(sql);
