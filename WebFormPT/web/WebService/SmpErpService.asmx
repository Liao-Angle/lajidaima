    ...
    public string createExpenseBillFlow(string poHeaderId, string subject)
    {
        ...
        if (!poHeaderId.Equals(""))
        {
            ...
            try
            {
                ...
                if (dsErp.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < dsErp.Tables[0].Rows.Count; j++)
                    {
						...
						if ((poCategory.Substring(0, 4).Equals("F1進口")) || (poCategory.Substring(0, 4).Equals("F2出貨")) || (poCategory.Substring(0, 4).Equals("F14其")) || (poCategory.Substring(0, 4).Equals("F3快遞"))) {
							if ((buyer.Equals("5010")) || (buyer.Equals("3583")) || (buyer.Equals("4098")) || (buyer.Equals("4878")) || (buyer.Equals("4868")) || (buyer.Equals("5067")))
                            {
                ...
                decimal funcNTAmount = Math.Round(enterNTAmount * rate , MidpointRounding.AwayFromZero);
                decimal funcTax = Math.Round(enterTax * rate , MidpointRounding.AwayFromZero);
                decimal funcAmount = Math.Round(funcNTAmount + funcTax , MidpointRounding.AwayFromZero);
