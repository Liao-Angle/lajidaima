...
function drawTreeRec(node, level)
{
    for(var x=0;x<node.childNodes.length;x++){
        ...
        if(node.childNodes[x].nodeName=='Folder'){
			...
			var inboxErp = "收件資料匣(ERP)"
            var itemId = "";
			
			if(title.indexOf(inboxErp) >= 0) {
                title = title.substr(0, inboxErp.length);
                itemId = "inboxErp";
            } else if(title.indexOf(inboxEf) >= 0) {
...
function clickItem(title, url, width, height, frameType, isMax)
{
    ...
    if(frameType=='1'){
		...
		var inboxErp = "收件資料匣(ERP)";
		if (title.indexOf(inboxErp) >= 0) {
            PageMethods.ProcessInboxErpCount(onSucessErp, onError); 
        } else if(title.indexOf(inboxEf) >= 0) {
...
function onSucessErp(result) {
    var html = document.getElementById('inboxErp').innerHTML;
    var idx = html.lastIndexOf("(");
    html = html.substr(0, idx);
    html = html + "("+result+")";
    document.getElementById('inboxErp').innerHTML = html;
}
