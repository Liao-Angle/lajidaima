...
public partial class Program_SCQ_Form_EH010902_PrintPage : BaseWebUI.GeneralWebPage  //System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string strSheetNo = (string)Session["EH010902_SheetNo"];
            if (Session["EH010902_SheetNo"] != null)
            {
                strSheetNo = (string)Session["EH010902_SheetNo"];
            }
            ...
            //顯示使用者資訊
            sql = @"SELECT   a.EmpName, a.PartName, a.zszg, a.DtName,Dt,EmpType,ComeDateY,TryDateB,TryDateE,zhize,bx,zgjg,zwjg,zyjg,qx,jbfjs,a.SheetNo, 
                    s.SMWYAAA006 as Subject, s.SMWYAAA008, s.SMWYAAA009, s.SMWYAAA010 
                    FROM EH0109A a, SMWYAAA s where s.SMWYAAA019=a.GUID and a.SheetNo='" + strSheetNo + "'";

            ds = engine.getDataSet(sql, "TEMP");
            if (ds.Tables[0].Rows.Count > 0)
            {
                EmpName.ValueText = ds.Tables[0].Rows[0]["EmpName"].ToString();
                PartName.ValueText = ds.Tables[0].Rows[0]["PartName"].ToString();
                zszg.ValueText = ds.Tables[0].Rows[0]["zszg"].ToString();
                DtName.ValueText = ds.Tables[0].Rows[0]["DtName"].ToString();
                Dt.ValueText = ds.Tables[0].Rows[0]["Dt"].ToString();
                EmpType.ValueText = ds.Tables[0].Rows[0]["EmpType"].ToString();
                ComeDateY.ValueText = ds.Tables[0].Rows[0]["ComeDateY"].ToString();
                TryDateB.ValueText = ds.Tables[0].Rows[0]["TryDateB"].ToString();
                TryDateE.ValueText = ds.Tables[0].Rows[0]["TryDateE"].ToString(); ;
                zhize.ValueText = ds.Tables[0].Rows[0]["zhize"].ToString();
                bx.ValueText = ds.Tables[0].Rows[0]["bx"].ToString();
                zgjg.ValueText = ds.Tables[0].Rows[0]["zgjg"].ToString();
                zwjg.ValueText = ds.Tables[0].Rows[0]["zwjg"].ToString();
                zyjg.ValueText = ds.Tables[0].Rows[0]["zyjg"].ToString();
                qx.ValueText = ds.Tables[0].Rows[0]["qx"].ToString();
                jbfjs.ValueText = ds.Tables[0].Rows[0]["jbfjs"].ToString();
            }
    ...
    public DataSet fh_sj(string SheetNo)
    {
		...
		sql = sql +  " from WebFormPT.dbo.EH0109A as sr ";
